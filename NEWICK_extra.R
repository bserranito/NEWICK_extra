...
test_func= function(Choix_tree){
  ...
  Name_file=basename(Choix_tree)



d = read.tree(file=Choix_tree)
...
#longueur des branches N2 inf?rieures au seuil
...
for (j in 1:dim(tN1_r)[1]){  #Nombre de Dichotomie et Trichotomie ...
  ...
  N1<-rbind(N1,hh) #Incr?mentation dans temp
}

# Boucle selection degr? 2 ------------------------------------------------

# R?cup?ration des num?ros de Noeuds r?pliqu?s du premier degr?
mat_NL=data.frame(mat_NL)
colnames(mat_NL)<- c("Label","Nods")
mat_NL$Nods.2=Edge[which(Edge[,2] %in% mat_NL$Nods),1]


# On recherche les niveaux terminaux non repliqu? au premier degr?
...
if (dim(D2)[1]==0){
  ...
  for (z in 1:dim(tN2)[1]){  #Nombre de Dichotomie et Trichotomie ...
    ...
    N2_2<-rbind(N2_2,hh2) #Incr?mentation dans temp
  }
  N2=rbind(N2_1,N2_2)
}
...
# On remplace la diagonale par des z?ros
...
addWorksheet(wb, "N1_table")
addWorksheet(wb, "N2_table")
addWorksheet(wb, "N1_matrix")
addWorksheet(wb, "N2_matrix")
addWorksheet(wb, "N1_prop")
addWorksheet(wb, "N2_prop")


# Table
writeData(wb, "N1_table", N1_table,startRow = 1, startCol = 1)
writeData(wb, "N2_table", N2_table,startRow = 1, startCol = 1)
# Matrix
writeData(wb, "N1_matrix", N1_Smat,rowNames=T)
writeData(wb, "N2_matrix", N2_Smat,rowNames=T)
#Prop Matrix
writeData(wb, "N1_prop", Prop.N1,rowNames=T)
writeData(wb, "N2_prop", Prop.N2,rowNames=T)

saveWorkbook(wb, file = paste0(Name_file,'_',"Output.xlsx"), overwrite = TRUE)
...
lapply(Choix_tree, FUN=test_func)
